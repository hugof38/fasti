...
impl BoundActActICMA<'_> {
    ...
    /// its period's reference dates, clamped to the schedule's span.
    fn ordered_year_fraction(&self, d1: Date, d2: Date) -> Fraction {
        let dates = self.schedule.dates();
        let refs = self.schedule.reference_dates();
        ...
        let mut total = Fraction::ZERO;
        for (w, r) in dates.windows(2).zip(refs.windows(2)) {
            if w[0] >= span.1 {
                break;
            }
            ...
            // Regular periods (r == w) reduce to a single ratio; stub
            // walks were shaped by the builder, so errors are confined
            // to range-edge pathologies where ZERO is the documented
            // fallback. Denominators come from the few distinct period
            // lengths in one schedule, so the sum stays in range.
            let chunk = self
                .inner
                .ordered_year_fraction(lo, hi, r[0], r[1])
                .unwrap_or_default();

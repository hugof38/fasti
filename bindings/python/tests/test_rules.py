...
def test_fixed_date_rule():
    ...
    assert rule.is_holiday(date(2026, 7, 4))
    assert not rule.is_holiday(date(2026, 7, 3))
    assert Rule.fixed(7, 4).is_holiday(date(2026, 7, 4))


def test_shift_is_applied_by_the_calendar_not_the_rule():
    rule = Rule.fixed("Jul", 4, shift="us")
    assert not rule.is_holiday(date(2026, 7, 3))
    assert Calendar.custom("us-ish", rules=[rule]).is_holiday(date(2026, 7, 3))
...
def test_nth_and_last_weekday_rules():
    ...
    assert memorial.is_holiday(date(2026, 5, 25))
    assert not memorial.is_holiday(date(2026, 5, 18))


def test_year_ranges_bound_a_rule():
    rule = Rule.fixed("Jun", 19, from_year=2022)
    assert rule.is_holiday(date(2022, 6, 19))
    assert not rule.is_holiday(date(2021, 6, 19))
    windowed = Rule.fixed("Jun", 19, from_year=2022, to_year=2024)
    assert not windowed.is_holiday(date(2025, 6, 19))
    assert Rule.fixed("Jun", 19, to_year=2024).is_holiday(date(1999, 6, 19))
...
def test_easter_offsets_count_from_easter_sunday():
    ...
    assert Rule.good_friday().is_holiday(easter - timedelta(days=2))
    assert Rule.easter_monday().is_holiday(easter + timedelta(days=1))
    assert Rule.ascension().is_holiday(easter + timedelta(days=39))
    assert Rule.whit_monday().is_holiday(easter + timedelta(days=50))
    assert Rule.corpus_christi().is_holiday(easter + timedelta(days=60))
...
def test_one_off_rule():
    rule = Rule.one_off(date(2026, 8, 14))
    assert rule.is_holiday(date(2026, 8, 14))
    assert not rule.is_holiday(date(2027, 8, 14))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        Rule.easter(-2),
        Rule.one_off(date(2026, 8, 14)),
    ],
    ids=repr,
)
def test_rules_describe_themselves_as_the_call_that_built_them(rule):
    rebuilt = eval(repr(rule), {"Rule": Rule, "datetime": __import__("datetime")})  # noqa: S307
...
def test_rules_can_extend_a_built_in_calendar():
    ...
    assert cal.is_holiday(date(2026, 10, 31))
    assert cal.is_holiday(date(2026, 7, 3))
    assert not fasti.calendars.US_SETTLEMENT.is_holiday(date(2026, 10, 31))
